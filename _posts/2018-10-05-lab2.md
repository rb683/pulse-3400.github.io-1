...
Despite the fact that there was talking in the background when we plotted our "no tone" frequency, there is clearly a difference in the output in bin 5. Thus, we're able to find a threshold value to distinguish the 660 Hz tone from the "no tone." In our fft_adc_serial, we added the following logic to turn on the LED when the 660 Hz tone was picked up by the microphone.
